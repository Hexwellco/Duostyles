...
function AppInner() {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn && !wasLoggedIn && route === '/') {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#f7f2fd' }}>
        <div className="animate-pulse text-[#9080b0] font-body text-sm">Loading...</div>
      </div>
    );
  }
  ...
  return <AppMain onNavigate={navigate} />;
}

function AppMain({ onNavigate }: { onNavigate: (path: string) => void }) {
      ...
      try {
        ...
        if (data.status === 'succeeded') {
          ...
          (async () => {
            ...
            if (session?.user && selectedRef) {
              await supabase.from('generations').insert({
                user_id: session.user.id,
                style_name: selectedRef.label,
